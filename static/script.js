document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const formData = {
            ...
            gender: inputs.gender.value, // 'male' or 'female'
            bmi: parseFloat(inputs.bmi.value),
            children: parseInt(inputs.children.value),
            smoker: smokerValue // 'yes' or 'no'
        };
        ...
        .then(data => {
            resultElement.innerText = `$${data.prediction.toFixed(2)}`;
            formErrorMessage.style.display = 'none';
        })
        .catch(error => {
            resultElement.innerText = '$---';
            formErrorMessage.innerText = 'Something went wrong.';
